/**
 * Webhook dispatch for the Solana indexer.
 *
...
export async function dispatchWebhooks(
  ...
  for (const wh of matching) {
    ...
    if (urlError) {
      console.warn(`[solana-webhooks] ${event} -> ${wh.url} rejected: ${urlError}`);
    ...
    try {
      ...
      console.error(`[solana-webhooks] delivery ${delivery.id} to ${wh.url} failed:`, err);
      await db
        .update(webhookDeliveries)
